...
> **Снимки пока не годятся для подачи, и это честнее сказать, чем показать.**
> У продукта нет визуального слоя: `pages.css` — 500 байт с комментарием «страницы
> намеренно голый HTML», а запись экранов говорит, что style pack **не выбран**.
> На первом запуске это уже не вопрос вкуса: без стилей статус рендерится встык
> с названием проверки — «Local storage**done**ready», — и экран нечитаем.
> Снимки обновляются после B-16.
