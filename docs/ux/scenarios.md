...
- **Status:** implemented
- **Known limit — the page can still delete the surface with script.** Removing the host node in a loop leaves no warning, measured 2026-08-20. Nothing about CSS or the element's name can reach it any more ([ADR-0001](../adr/0001-closed-shadow-root.md), amended), and this last vector needs a bounded re-mount with a journalled give-up rather than an unbounded fight with the page — tracked as B-68
- **Coverage:** apps/extension/src/content/collect.ts:collect, packages/core-injection/src/stage1.ts:detectHidden, packages/ui/src/banner/banner.ts:mountBanner, packages/ui/src/host.ts:createOverlayHost, e2e/scn-003.spec.ts, e2e/hostile-page.spec.ts
...
- **Known limit — the suffix table is a curated subset of the Public Suffix List.** A suffix it does not carry is read as a single label, which makes the registrable domain one label too short. That direction can only **miss** a finding, never invent one, which is why the subset is acceptable and the direction is stated. Vendoring the real list is B-66
- **Known limit — a brand under an unrelated top-level domain is not reported.** `paypal.security` and `paypal.support` pass silently. Nothing on the device separates a brand's own country domain from a squatter's gTLD: ownership is not a fact a content script has, and the rule that used to catch them — same name, any different ending — flagged `google.de`, `yandex.com`, `github.io`, `stripe.dev`, `discord.gg`, `sberbank.com`, `telegram.me`, `vk.ru` and `ozon.by`, every one of them the real company. What is kept is the **mistyped** ending: `.co` and `.cm` are one edit from `.com` and are the classic squats. The trade is the one [ADR-0012](../adr/0012-one-signal-is-a-suspicion.md) states in general — a warning on `google.de` is how a user learns to dismiss the next one. Recovering the coverage needs data that can decide, and that is B-67
...
- **Status:** implemented
- **Coverage:** packages/core-lookalike/src/check.ts:checkLookalike, packages/core-lookalike/src/suffix.ts:registrableDomain, packages/core-lookalike/src/real-hosts.test.ts, apps/extension/src/content/lookalike.ts:warnIfLookalike, e2e/scn-006.spec.ts
...
- **Status:** implemented
- **Coverage:** packages/core-feeds/src/rules.ts:buildRules, packages/ui/src/interstitial/interstitial.ts:renderInterstitial, apps/extension/src/interstitial/appeal-link.ts:appealLinkFor, apps/extension/src/interstitial/context.ts:settleContext, e2e/scn-007.spec.ts
