...
- **The name and the type are compared in both directions.** It fired one way round only: a name that looks executable while the server calls it a document. The commoner shape — `invoice.pdf` served as `application/x-msdownload` — passed silently. `application/octet-stream` counts only against a document-shaped name, because half the web sends it for anything it cannot classify
- **Status:** implemented
